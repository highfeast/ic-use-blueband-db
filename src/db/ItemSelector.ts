import { MetadataFilter, MetadataTypes } from "../utils/types";
...
export class ItemSelector {
   ...
   * @returns Similarity between the two vectors
   */
  public static normalizedCosineSimilarity(
    vector1: number[],
    norm1: number,
    vector2: number[],
    norm2: number
  ...
  public static select(
    ...
    for (const key in filter) {
      switch (key) {
        case "$and":
          if (
            !filter[key]!.every((f: MetadataFilter) => this.select(metadata, f))
          ) {
            return false;
          }
          break;
        case "$or":
          if (
            !filter[key]!.some((f: MetadataFilter) => this.select(metadata, f))
          ) {
            return false;
          }
          break;
      ...
      switch (key) {
        ...
        case "$in":
          if (typeof value == "boolean") {
            ...
            !filter[key]!.some(
              (val: any) =>
                typeof val == "string" && val.includes(value as string)
            )
          ) {
            return false;
          }
